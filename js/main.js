...
function animateStackCards() {
    var top = this.element.getBoundingClientRect().top;
    var offsetTop = 100, // Controls when animation starts
        cardHeight = 200,
        marginY = 15; // Initial margin between cards

    var lastCardIndex = this.items.length - 1;
   console.log(lastCardIndex);
    for (var i = 0; i < this.items.length; i++) {
        var scrolling = offsetTop - top - i * (cardHeight + marginY);

        if (scrolling > 0) {
            // Normal transition effect before last card appears
            var translateY = marginY * i;
            var scale = (cardHeight - scrolling * 0.05) / cardHeight;

            // If last card is fully visible, apply overlap effect
            if (i === lastCardIndex && scrolling >= 0) {
                this.items[i].setAttribute(
                    "style",
                    "transform: translateY(0px) scale(1); z-index: " + (this.items.length - i) + ";"
                );
            }
